...
-- Adds: PARTITION BY epoch, column codecs, TTL
-- =============================================================
...
-- Step 4: Re-add secondary indexes
...
-- Run the application's schema initialization (GetSchemaStatements) to recreate MVs.
-- Or restart the application - it will recreate them on startup.

-- Step 6: After verifying everything works, drop old tables
...
-- TTL is applied via the new table definition above.
-- For existing tables not migrated, apply TTL with:
-- ALTER TABLE qubic.flow_tracking_state MODIFY TTL toDateTime(updated_at) + INTERVAL 90 DAY WHERE is_complete = 1;
