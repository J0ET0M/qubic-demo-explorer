...
public class ClickHouseQueryService : IDisposable
{
    ...
    /// <summary>
    /// Save current holder distribution as a historical snapshot
    /// </summary>
    ...
    /// </summary>
    private async Task<(ulong NewAddresses, ulong ReturningAddresses)> GetNewVsReturningForEpochAsync(
        uint epoch, CancellationToken ct)
    {
        await using var cmd = _connection.CreateCommand();
        cmd.CommandText = $@"
            WITH first_appearance AS (
    ...
    /// </summary>
    private async Task<(decimal InflowVolume, ulong InflowCount, decimal OutflowVolume, ulong OutflowCount)>
        GetExchangeFlowsForEpochAsync(uint epoch, CancellationToken ct)
    {
        await _labelService.EnsureFreshDataAsync();
        var exchangeAddresses = _labelService.GetAddressesByType(AddressType.Exchange);
    ...
    /// </summary>
    private async Task<(ulong NewAddresses, ulong ReturningAddresses)> GetNewVsReturningForTickRangeAsync(
        ulong tickStart, ulong tickEnd, CancellationToken ct)
    {
        await using var cmd = _connection.CreateCommand();
        cmd.CommandText = $@"
            WITH first_appearance AS (
    ...
    /// </summary>
    private async Task<(decimal InflowVolume, ulong InflowCount, decimal OutflowVolume, ulong OutflowCount)>
        GetExchangeFlowsForTickRangeAsync(ulong tickStart, ulong tickEnd, CancellationToken ct)
    {
        await _labelService.EnsureFreshDataAsync();
        var exchangeAddresses = _labelService.GetAddressesByType(AddressType.Exchange);
    ...
    /// </summary>
    private async Task<(ulong Users100MPlus, ulong Users1BPlus, ulong Users10BPlus)>
        GetNewUsersWithHighBalanceForEpochAsync(uint epoch, CancellationToken ct)
    {
        await using var cmd = _connection.CreateCommand();
    ...
    /// </summary>
    private async Task<(ulong Users100MPlus, ulong Users1BPlus, ulong Users10BPlus)>
        GetNewUsersWithHighBalanceForTickRangeAsync(ulong tickStart, ulong tickEnd, CancellationToken ct)
    {
        await using var cmd = _connection.CreateCommand();
    ...
    /// <summary>
    /// Calculates total emission (inflow FROM zero address) for computor addresses in a tick range.
    /// This is the epoch reward distribution that happens at the end of each epoch.
    /// </summary>
    public async Task<ComputorFlowService.FlowSummary> CalculateEmissionForComputorsAsync(
        HashSet<string> computorAddresses,
        ulong tickStart,
        ulong tickEnd,
        CancellationToken ct = default)
    {
        if (computorAddresses.Count == 0)
            return new ComputorFlowService.FlowSummary(0, 0);

        var addressList = string.Join(",", computorAddresses.Select(a => $"'{EscapeSql(a)}'"));

        await using var cmd = _connection.CreateCommand();
        // Emission = transfers FROM zero address TO computors
        cmd.CommandText = $@"
            SELECT
    ...
    /// <summary>
    /// Calculates total inflow TO computor addresses in a tick range (from any source except zero address).
    /// This captures additional transfers to computors during the epoch (not emission).
    /// </summary>
    ...
    /// Excludes transfers TO zero address and 1M deposit/return transfers.
    /// </summary>
    ...
    /// <summary>
    /// Saves flow hop records to database
    /// </summary>
